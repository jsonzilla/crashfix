...
bool CSocketServer::Init(
		CDaemon* pDaemon,
		const int nPort,
		const int nMaxQueueSize,
		const int nTotalThreads,
		const int nMaxMemUsageMB,
		CLog* pLog
)
{
...
void CSocketServer::Run()
{
	...
	while (1)
	{
		...
		AddRequest(client_sock, nullptr, nullptr);
	}
}
...
int CSocketServer::SendMsgWithTimeout(SOCK sock, const char* msg, double timeout, std::string& sErrorMsg)
{
	...
	// Get current time
	const double write_start_time = microtime();
	...
	while(bytes_written<nMsgLen)
	{
		...
		int retval = select(nfds, nullptr, &wfds, nullptr, &tv);
		if(retval < 1)
		{
		...
		// Get current time
		const double cur_time = microtime();

		// Get duration in seconds
		const double duration { (cur_time - write_start_time) / 1000 };
		if(duration>timeout)
		{
...
int CSocketServer::RecvMsgWithTimeout(SOCK sock, double timeout, unsigned uMaxSize, std::string& sMsg, std::string& sError)
{
	...
	while(1) {
		...
		nfds = sock+1;
#endif
		nRetVal = select(nfds, &rfds, nullptr, nullptr, &tv);
		if(nRetVal < 1)
		{
		...
		// Get duration in seconds
		const double duration { (cur_time - read_start_time) / 1000 };
		if(duration>5)
		{
...
int CSocketServer::GetServerStatus(std::string& err_msg)
{
	...
	const int nErrorCount = m_pDaemon->GetErrorCount();
	...
	for(int i=0; i< m_nTotalThreads; i++)
	{
		...
		char szDateTime[64] = "";
		const struct tm * const timeinfo = gmtime( &StartTime );
		if(timeinfo)
...
bool CSocketServer::AddRequest(SOCK& Sock, const char* szCommand, std::string* psCmdId)
{
	...
	else
	{
		...
		const int nReadyThreads { m_nTotalThreads - m_nBusyThreads };
...
{
	// Validate input
	if(szCmdId==nullptr || szCommand==nullptr) {
		return (false);
	}
	else {
		CAutoLock lock(&m_Lock);
...
bool CSocketServer::UpdateAssyncCommandInfoOnCompletion(
		const char* const szCmdId,
		const int nCompletionStatus,
		std::string& sErrorMsg
)
{
	// Acquire synchronisation lock.
...
int CSocketServer::GetAssyncCommandInfo(std::string& sErrorMsg, const char* szCommandId, bool bEraseCompleted)
{
	...
	{
		if(szCommandId!=nullptr && szCommandId!=it->first)
			continue;
